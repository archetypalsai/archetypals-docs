...
_The following risks are relevant to any AI using your computer, not just Archetypals_
...
The <b>Computer use</b> feature for Archetypals is an experimental feature that allows you to enable an AI to use your computer to complete tasks.
...
- **Accessibility:** (MacOS only) This feature requires the `Accessibility` and `Screen Recording` permissions to be enabled for Archetypals.
...
You can do this by clicking the pause icon in the UI, pressing `CMD+K` or `CTRL+K`, or by quitting the Archetypals application.
...
_This section is relevant to users running Archetypals Desktop on MacOS_
...
In order to use the computer use feature, you need to have the `Accessibility` permissions enabled for Archetypals on your system.

This is done by opening the `Security & Privacy` settings on MacOS and clicking on the `Privacy` tab. From there, find `Accessibility` on the left and click on the `+` button to add Archetypals.

This will allow Archetypals to control your computer's mouse and keyboard.
...
In order to use the computer use feature, you need to have the `Screen Recording` permissions enabled for Archetypals on your system.

This is done by opening the `Security & Privacy` settings on MacOS and clicking on the `Privacy` tab. From there, find `Screen Recording` on the left and click on the `+` button to add Archetypals.

This will allow Archetypals to take screenshots of your display.
...
<Callout type="info">
  **Note:** Be ready at any time to abort the computer use session if things are
  ...
  Archetypals application.
</Callout>

Once you have enabled the feature and configured it with your API key, you can invoke computer use by typing in `@os` in the Archetypals chat along with a prompt.
...
Once the OS agent is running, Archetypals will minimize to get out of the way and you should see a popup in your display allowing you to control or halt the OS agent.
...
You can also quit the Archetypals application which will halt the OS agent as well. You can drag the popup around to get it out of the way, but this may interfere with the OS agent's ability to control your mouse position if needed.
...
The OS agent will output its actions and any relevant information to the Archetypals chat as it executes. These actions are currently **not** saved or stored your workspace's chat history.
...
(Need to update this line because of Discord) If you are interested in helping us work on this, please reach out to us on [Discord](https://discord.gg/Dh4zSZCdsC) and we can talk about how you can help!
